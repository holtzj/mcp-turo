...
export async function getCarDetails(listingId) {
    ...
    try {
        ...
        const details = await page.evaluate((id) => {
            ...
            // Title / name
            const titleEl = document.querySelector('h1, [class*="vehicleName"], [class*="vehicle-name"]');
            ...
            // Price
            const priceEl = document.querySelector('[class*="price"], [class*="dailyRate"]');
            ...
            // Trip count
            const tripEl = document.querySelector('[class*="trip"]');
            ...
            // Description
            const descEl = document.querySelector('[class*="description"], [class*="about"]');
            const description = descEl?.textContent?.trim() || "";
            // Host
            const hostNameEl = document.querySelector('[class*="hostName"], [class*="host-name"]');
            const host_name = hostNameEl?.textContent?.trim() || "Host";
            const hostLinkEl = document.querySelector('a[href*="/profile/"]');
            ...
            document.querySelectorAll('[class*="spec"], [class*="detail"]').forEach((el) => {
                const labelEl = el.querySelector('[class*="label"], dt');
                const valueEl = el.querySelector('[class*="value"], dd');
                if (labelEl && valueEl) {
            ...
            document
                ...
                .forEach((el) => {
                const text = el.textContent?.trim();
                if (text && !guidelines.includes(text))
            ...
            // Cancellation policy
            const cancelEl = document.querySelector('[class*="cancellation"], [class*="cancel"]');
...
//# sourceMappingURL=get_car_details.js.map