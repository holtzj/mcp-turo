...
export async function createBooking(params) {
    ...
    try {
        ...
        const isLoggedIn = await page.evaluate(() => {
            const loginEl = document.querySelector('[class*="login"], [class*="sign-in"], [href*="/login"]');
            return !loginEl;
        });
        if (!isLoggedIn) {
            return {
        ...
        const priceInfo = await page.evaluate(() => {
            ...
            document
                ...
                .forEach((item) => {
                const labelEl = item.querySelector('[class*="label"], [class*="name"], span:first-child');
                const valueEl = item.querySelector('[class*="value"], [class*="amount"], span:last-child');
                if (labelEl && valueEl) {
            ...
            const dailyRateEl = document.querySelector('[class*="dailyRate"], [class*="daily-rate"]');
        ...
        const bookButton = await page.$('[data-testid="book-button"], [class*="bookButton"], button[class*="book"]');
        if (!bookButton) {
            return {
                booking_id: "",
                status: "unavailable",
        ...
        if (params.message_to_host) {
            const messageBox = await page.$('textarea[class*="message"], textarea[placeholder*="message"], [class*="messageHost"] textarea');
            if (messageBox) {
        ...
        const confirmation = await page.evaluate(() => {
            ...
            const statusEl = document.querySelector('[class*="status"], [class*="confirmation"]');
...
function buildBreakdown(priceInfo, days) {
